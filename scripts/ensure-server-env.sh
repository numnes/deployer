#!/usr/bin/env bash
# Create or update server/.env with runtime connection settings.
set -euo pipefail
...
usage() {
  echo "Usage: ensure-server-env.sh --api-port PORT --postgres-port PORT --redis-port PORT --front-port PORT" >&2
  exit 1
}
...
[[ -n "$API_PORT" && -n "$POSTGRES_PORT" && -n "$REDIS_PORT" && -n "$FRONT_PORT" ]] || usage
...
set_env_var CORS_ORIGIN "http://localhost:${FRONT_PORT}"

echo "[ensure-env] server/.env updated (API :${API_PORT}, Postgres :${POSTGRES_PORT}, Redis :${REDIS_PORT}, Front :${FRONT_PORT})"
